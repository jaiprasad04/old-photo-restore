...
export default function Gallery() {
  ...
  const fetchCreations = async () => {
    try {
      ...
      console.error("Error fetching restorations:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id, e) => {
    e.stopPropagation();
    if (!confirm("Are you sure you want to delete this restoration?")) return;
  ...
  const handleDownload = async (imageUrl, filename) => {
    ...
    try {
      ...
      a.href = url;
      a.download = filename || `vintage-restore-${Date.now()}.png`;
    ...
    return (
      <main className="flex-1 flex flex-col items-center justify-center p-6 bg-transparent">
        <div className="max-w-md text-center space-y-6">
          ...
          <h1 className="text-3xl font-extrabold text-primary-text tracking-tight">Restorations Gallery</h1>
          <p className="text-secondary-text text-sm leading-relaxed">
            Please sign in to view your collection of restored historical photos.
          </p>
          <button
  ...
  return (
    <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 w-full bg-transparent overflow-y-auto scrollbar-subtle">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-divider pb-6 mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-primary-text tracking-tight">Your Restorations</h1>
          <p className="text-xs text-secondary-text mt-1 font-medium">A portfolio of all your restored historical photos.</p>
        </div>
      ...
      {creations.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-16 text-center border border-divider rounded-2xl bg-bg-card/20">
          ...
          <span className="text-sm font-bold text-primary-text">No photos restored yet</span>
          <p className="text-xs text-secondary-text mt-2 max-w-[200px] leading-relaxed">
            Head to the workspace editor and start restoring your custom memories.
          </p>
        </div>
      ) : (
            ...
            >
              <div className="aspect-[3/4] w-full relative overflow-hidden bg-black/40">
                {item.status === "processing" ? (
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-4 bg-black/40">
                    <span className="text-red-500 font-bold text-xs mb-2">Restoration Failed</span>
                    <button
            ...
            <div className="space-y-4 pt-2">
              <div>
                ...
                <div className="flex gap-4 text-[10px] text-secondary-text mt-2 font-semibold">
                  <span>Mode: {activeCreation.mode}</span>
              ...
              <div className="flex gap-2 pt-2">
                <button
                  onClick={() => handleDownload(activeCreation.outputUrl, `vintage-restore-${activeCreation.id}.png`)}
